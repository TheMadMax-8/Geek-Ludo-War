...
from ml.rl_agent import get_cognitive_flow_adjustment, reward_agent
...
def handle_submission_success(data):
    ...
    reward_agent(v.get('user_id'), user_succeeded = True, next_frustration_skips = 0) 
...
def handle_hack_attempt(data):
    ...
    if action == 'skip':
        ...
        reward_agent(hacker.get('user_id'), user_succeeded = False, next_frustration_skips = 3) # Agent Intervened Still, User Hit The Frustration Wall. High Frustration Signal To Agent.
    elif action == 'hack':
